/**
 * Sales Scores Detail Section Component
 *
 * Full-page section displaying 5 sales strategy metrics
 * Replaces the Dialog popup from SalesScoreCard
 */
...
interface SalesScoresDetailSectionProps {
  metrics: ScoreMetric[];
  totalScore: number;
  maxScore: number;
...
function TimestampBadge({
  ...
  return (
    <Badge
      variant="outline"
      className="cursor-pointer hover:bg-purple-100 hover:border-purple-500 transition-colors"
      onClick={(e) => {
        e.stopPropagation();
        onClick?.(timestamp);
      }}
...
export function SalesScoresDetailSection({
  metrics,
  totalScore,
  maxScore,
  onTimestampClick,
}: SalesScoresDetailSectionProps) {
  ...
  return (
    <Card id="sales-scores-detail" className="shadow-sm">
