...
#### 需求背景
用戶反饋課程進度表格的彈窗功能需要優化：
1. 軌道/支點/氣息彈窗應顯示：學員名稱 + 卡片名稱 + 完成日期
2. 卡片週變化彈窗應顯示：本週 vs 上週卡片對比
3. 學員變化彈窗應顯示：本週 vs 上週學員對比
4. 「全部」篩選時應隱藏卡片週變化和學員變化欄位

#### 實作內容

**1. 軌道/支點/氣息彈窗格式優化**
- 修改顯示格式：學員名稱 + 卡片名稱 + 完成日期
- 新增排序功能（點擊表頭可排序）
- 預設按完成日期降序排列

**2. 卡片週變化彈窗（本週 vs 上週對比）**
- 後端新增週期計算邏輯（週四~週三為一週）
- 顯示欄位：期間（Badge）、學員、卡片名稱、完成日期
- 本週資料在前，上週資料在後
- Karen 範例：本週 8 張 vs 上週 9 張 = -1

**3. 學員變化彈窗（本週 vs 上週對比）**
- 顯示欄位：期間、學員、本期完成卡片數、狀態
- 狀態標籤：
  - 「新增」：本週新加入的學員
  - 「持續」：兩週都有活躍的學員
  - 「減少」：上週有但本週沒有的學員
...
**4. 條件式欄位顯示**
- 新增 `showAlways` 屬性控制欄位顯示
- 「全部」篩選時隱藏「卡片週變化」和「學員變化」欄位

#### 修改檔案
- [`client/src/pages/teaching/course-progress.tsx`](client/src/pages/teaching/course-progress.tsx) - 前端彈窗顯示邏輯
- [`server/services/trello-sync-service.ts`](server/services/trello-sync-service.ts) - 後端週期對比查詢
...
- Runtime Logs 顯示：`NotTriggerScaleUp: pod didn't trigger scale-up:`
- 連續多次部署都失敗

#### 根因分析
**`pg` (node-postgres) 模組沒有被列在 `package.json` 的 dependencies 中**

- 本機開發時，`pg` 可能是某個套件的間接依賴，所以可以正常運作
- Zeabur 執行 `npm ci --omit=dev` 時，沒有安裝 `pg` 模組
- 應用程式啟動時找不到 `pg` 模組而 crash
...
#### 預防措施文檔
新增 [`docs/ZEABUR_DEPLOYMENT_ISSUES.md`](docs/ZEABUR_DEPLOYMENT_ISSUES.md) 記錄部署問題歷史和排查方法
...
#### 問題
課程進度頁面顯示 "11/27~12/3" 為第 48 週，但應該是第 49 週

#### 原因
使用 ISO 8601 週數計算，與用戶預期的「從年初開始計算經過幾週」不同

#### 修正
從 ISO 週數計算改為簡單的經過週數計算：
...
#### 相關 Commit
- `a83ed81` - fix: 修正週數計算 - 11/27~12/3 顯示為第49週
...
#### 需求背景
用戶希望：
1. 「我想看到他們個別完成的卡片是哪些」- 查看每位學員完成的具體卡片明細
2. 「我想要可以一次看到每個老師每週的進度」- 查看所有老師的總覽統計

#### 資料庫變更

**新增 `teacher_card_completions` 表**（儲存個別卡片完成記錄）

```sql
CREATE TABLE teacher_card_completions (
  id UUID PRIMARY KEY,
  progress_id UUID REFERENCES teacher_course_progress(id),
  card_id VARCHAR(255),           -- Trello 卡片 ID
  card_name VARCHAR(500),         -- 卡片名稱
  module_name VARCHAR(100),       -- 模組名稱（軌道/支點/氣息）
  card_order INTEGER,             -- 卡片順序
  completed_at TIMESTAMP,         -- 完成時間
  UNIQUE(progress_id, card_id)    -- 防止重複記錄
);
```

**新增 UNIQUE 約束**（防止重複 Board）
```sql
...
**1. 取得學員卡片完成明細** ([`routes.ts:8372-8416`](server/routes.ts#L8372-L8416))
```
GET /api/course-progress/:progressId/cards
```
回傳該學員所有完成的卡片，包含：卡片名稱、模組、完成日期

**2. 取得老師總覽統計** ([`routes.ts:8418-8465`](server/routes.ts#L8418-L8465))
```
GET /api/course-progress/teacher-summary
```
回傳所有老師的統計數據：學員數、總完成卡片數、平均進度

#### 前端 UI 改進

**課程進度頁面重構** ([`course-progress.tsx`](client/src/pages/teaching/course-progress.tsx))

1. **雙 Tab 設計**
   - 「學員進度」Tab：顯示所有學員的進度表格
   - 「老師總覽」Tab：顯示各老師的統計數據

2. **可展開的卡片明細**
   - 點擊學員列可展開查看完成的卡片清單
   - 使用 Collapsible 組件實現平滑展開動畫
   - 延遲載入：只有展開時才呼叫 API 取得卡片資料
...
**修復 `nickname` 欄位不存在錯誤** ([`routes.ts:8317-8331`](server/routes.ts#L8317-L8331))

問題：`column u.nickname does not exist`
原因：`users` 表沒有 `nickname` 欄位

修復：改用 `business_identities.display_name`
```typescript
...
1. **延遲載入優化**：卡片明細只在展開時載入，避免一次載入大量資料
2. **UNIQUE 約束**：防止重複 Board 記錄，確保資料完整性
3. **模組識別**：根據 Trello List 名稱自動識別模組（軌道/支點/氣息）
...
#### 背景
原本的三階段分潤制度（30%/40%/30%）有公平性問題，改為「固定卡片價值 + 模組完成獎勵」的新架構。

#### 新制度架構

**分潤流程**：
```
學生購買 → closer 判定 → 銷售獎金/諮詢師抽成 → 教學分潤 → 模組獎勵
```

**關鍵規則**：
1. `closer = 老師本人` → 老師拿 8% 銷售獎金 + $770/卡 教學分潤
2. `closer = 諮詢師` → 諮詢師拿 8.8% 抽成，老師拿 $654/卡 教學分潤
3. 模組獎勵：不管誰成交，都是教學老師拿

**金額對照表**：

| 購買方案 | 成交者 | 銷售獎金 | 諮詢師抽成 | 教學分潤 | 模組獎勵 | 老師總計 | 諮詢師總計 |
|---------|--------|---------|----------|---------|---------|---------|-----------|
...
#### 邊界情況處理
- **成交判定**：以 `closer` 欄位為準
- **學生換老師**：卡片跟人走，模組獎勵給完成者
- **退費處理**：7 天內追回銷售獎金，已教課程不追回
- **新舊交接**：以教學日期判斷，2025/01/01 後用新制

#### 文檔
- 完整設計文檔：[`docs/TEACHER_COMMISSION_SYSTEM_2025.md`](docs/TEACHER_COMMISSION_SYSTEM_2025.md)

#### 待實作項目（2025 年 1 月）
1. 資料庫 Migration：`teacher_course_progress` 表
2. 後端：新增分潤計算函數
3. 前端：薪資計算頁面更新
4. Trello 整合：卡片追蹤自動化
